...
class Problem:
    ...
    def pre_jit_fns(self):
        ...
        if hasattr(self, 'get_surface_maps'):
            assert num_surfaces == len(self.get_surface_maps()), f"Mismatched number of surfaces: {num_surfaces} != {len(self.get_surface_maps())}"
        elif hasattr(self, 'get_universal_kernels_surface'):
            assert num_surfaces == len(self.get_universal_kernels_surface()) , f"Mismatched number of surfaces: {num_surfaces} != {len(self.get_universal_kernels_surface())}"
        else:
